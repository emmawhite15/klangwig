...
- These are differences between groups (or between group averages) for *categorical* predictors
   -   Refresh: categorical predictors are referenced against a
        baseline (labeled Intercept)


## Coding for categorical predictors: contrasts

- What do the parameters of a linear model mean?
  - other names for parameters: coefs, slopes (continuous), $\beta$
...
R's behavior may seem annoying at first -- it seems like the estimated values of the groups are what we're really interested in -- but it is really designed for testing *differences among groups*. To get the estimates per group, you could:
...
Some data on lizard perching behavior (`brglm` package; Schoener 1970 *Ecology* **51**:408-418). 
...
Response (Y variable) is number of *Anolis grahami* lizards found on perches in particular conditions.
...
What is the effect of `light` AND `time` on lizard perching? (assuming these have indepdendent effects)
```{r lizardTL1}
...
- The hypothesis here is that the effect of one variable depends on the presence of another
  - This is often true in science! What are some examples?
...
-   For example, on a very sunny day, there might be fewer lizards
    perching at midday because it is very hot; whereas on a cloudy day, the number of lizards might be highest at midday
...
- It asks: **does the number of lizards perching depend jointly on time of day and light?**
...
-   We can use emmeans and effects and get a pretty good idea
    what's going on
-   In more complicated cases, interaction terms may have many
    parameters
